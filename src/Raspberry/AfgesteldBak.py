...
def checkBoolean(lockBool):
    ...
    if randomslot == 1 or randomslot == 3:
        if lockBool == False and angle == 80:
            print("turn to 140 is open, led is on")
            if  angle != 140:
                GPIO.output(pinList[randomslot-1],GPIO.HIGH)
                SetAngle(140)
                angle = 140
        elif lockBool == True and  angle == 140:
            print("turn to 80 is closed, led is off")
            if  angle != 80:
                GPIO.output(pinList[randomslot-1],GPIO.LOW)
                SetAngle(80)
                angle = 80
    elif randomslot == 2 or randomslot == 4:
        if lockBool == True and angleEven == 100:
            print("turn to 150 is closed, led is off")
            if  angleEven != 150:
                GPIO.output(pinList[randomslot-1],GPIO.HIGH)
                SetAngle(150)
                angleEven = 150
        elif lockBool == False and  angleEven == 150:
