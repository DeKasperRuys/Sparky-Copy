...
def putAvailablePowerbank(aPowerbank):
    ...
    myobj = {}
    myobj['id'] = aPowerbank
    x = requests.put(url, data=json.dumps(myobj),headers=headers)#,verify=False
    print(x.text)
...
def pollLoanRequest(aPowerbank):
    ...
    if str(response) == correct:
        if object_json == False:
            ...
            print ("loanrequest is false , aPowerbank set to 0")
            aPowerbank = 0
...
def pollReturnRequest():
    response = requests.get('https://sparky1920api.azurewebsites.net/api/stations/poll/isreturnrequest/'+ str(stationId), headers=headers, )
    object_json = requests.get('https://sparky1920api.azurewebsites.net/api/stations/poll/isreturnrequest/' + str(stationId), headers=headers).json()
    correct = "<Response [200]>"
    if str(response) == correct:
        if object_json == False:
            print("Return request is false")
            time.sleep(sleepTime)
        else:
            print("Return request is true")
            time.sleep(sleepTime)
