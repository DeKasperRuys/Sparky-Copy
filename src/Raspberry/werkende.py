...
def getPowerbank():
    global randomslot
    global aPowerbank
    #backend geeft mij de volste powerbank
    #backend geeft mij het slotnummer op basis van powerbankID
    response = requests.get('https://sparky1920api.azurewebsites.net/api/stations/get-fullestpwb/'+ str(stationId), headers=headers,verify = False)
    object_json = requests.get('https://sparky1920api.azurewebsites.net/api/stations/get-fullestpwb/' + str(stationId), headers=headers, verify = False).json()
    randomslot = object_json['slotId']
    aPowerbank = object_json['powerbankId']
    #tijdelijk random
    #random slot waarde tussen 1 ,3, 4 , 2 werkt niet
    #randomlist = [1,3,4]
    #randomslot = random.choice(randomlist)
    print("geselecteerde slot is" +str(randomslot))
    # waarde tussen 1 en 4
    #aPowerbank = random.randint(1,4)
    #aPowerbank = 6
    print("geselecteerde powerbank is" +str(aPowerbank))
...
def updateSlot(rfidBank,emptyslot):
    ...
    #slot meegeven ook
    myobj['slotId'] =  emptyslot
    x = requests.put(url, data=json.dumps(myobj),headers=headers,verify = False)#,verify=False
    print(x.text)
    print(str(emptyslot)+ " " + str(rfidBank)+ "updated")
    time.sleep(sleepTime)
...
def emptySlot(slot):
    baseurl = 'https://sparky1920api.azurewebsites.net/api/stations/set-empty-slot'
    url = baseurl + str(stationId)
    print (url)
    # zoeken hoe ik available powerbank hier in krijg altijd een andere waarde
    myobj = {}
...
def pollLoanRequest():
    ...
    if str(response) == correct:
        if object_json == False:
            ...
            if selectedBool == None or selectedBool == False:
                getPowerbank()
                time.sleep(sleepTime)
                selectedBool = True
...
def pollOpenStation():
    ...
    if str(response) == correct:
        if object_json == False:
            ...
            #slot leeg setten
            pwb = 0
            updateSlot(pwb,randomslot)
            #emptySlot(randomslot)
            time.sleep(sleepTime)
    ...
    if str(response) == correct:
        if object_json == False:
            ...
            if emptyslot == 0:
                ...
                if boolPB == True:
                    rfidBank = setPB
                    updateSlot(rfidBank,emptyslot)
                    print ("Put Return OK")
                    putReturnOk()
                    # reset rfid en slot en ledje rfid
                    boolPB = False
                    setPB = ""
                    rfidText = ""
                ...
                print("rfid al reeds ingevuld wachten op pay")
                time.sleep(sleepTime)
